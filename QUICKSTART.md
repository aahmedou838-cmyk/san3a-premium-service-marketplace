...
## ☁️ الطريقة الأسهل — GitHub Codespaces (بدون تحميل شيء)

شغّل التطبيق كاملاً في متصفحك من خلال GitHub مباشرة:

### الخطوات:

1. اذهب إلى: **https://github.com/aahmedou838-cmyk/san3a-premium-service-marketplace**
2. اضغط الزر الأخضر **`Code`** أعلى يمين قائمة الملفات
3. اختر تبويب **`Codespaces`**
4. اضغط **`Create codespace on main`**
5. انتظر دقيقتين — سيُنشئ GitHub بيئة تطوير كاملة في السحابة
6. سيفتح VS Code في متصفحك مع كل الأدوات مُثبّتة (Bun + Node)

### داخل Codespaces:

في الـ terminal السفلي، شغّل (بالترتيب):

```bash
# 1) إعداد Convex backend (مرة واحدة)
bunx convex login
bunx convex dev
# اختر "Create a new project" وسمّه san3a
# اتركه يعمل في هذه النافذة
```

افتح **terminal ثانية** (اضغط `+` في أعلى منطقة الـ terminal) ثم:
...
ستظهر نافذة منبثقة في أسفل VS Code تعرض رابط التطبيق — اضغط **`Open in Browser`** وسيفتح التطبيق!

### الميزات في Codespaces:
- ✅ كل الأدوات مُثبّتة تلقائياً (Bun, Node, Git, VS Code Extensions)
- ✅ لا حاجة لتثبيت شيء على جهازك
- ✅ يعمل من أي جهاز حتى من التابلت
- ✅ 60 ساعة مجانية شهرياً (حساب GitHub عادي)
- ❌ بناء تطبيق Android/iOS لا يعمل هنا (يحتاج Android Studio/Xcode)

---

## 💻 الطريقة الثانية — التشغيل المحلي

إذا فضّلت التشغيل على جهازك:
